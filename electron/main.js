...
function findPython() {
  if (process.env.PYTHON_PATH) return process.env.PYTHON_PATH; // override opcional
  const bundled = process.platform === 'win32'
    ? path.join(process.resourcesPath, 'python', 'python.exe')
    : path.join(process.resourcesPath, 'python', 'bin', 'python3');
  if (fs.existsSync(bundled)) return bundled;
  return process.platform === 'win32' ? 'python.exe' : 'python3'; // fallback al Python del sistema
}
...
function spawnWallet(pyPath) {
  const script = path.join(__dirname, '..', 'wallet.py'); // <- tu archivo
